...
The main limitation of the present approach may be found in the client-side transformation of the 2.5D cellular model performed via simple polyhedral extrusions of chain descriptions made by polygons and polylines. This 3D model generation is not extendable to more unusual architectures, like, e.g.~hanging structures. Anyway, even the most creative shapes could be directly modeled server-side, and inserted within HIJSON files via the same mechanism currently used for furnitures and smart-devices. We plan to inser this generalization with the publication of the HIJSON format.
