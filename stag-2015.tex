...
\title[SUBMISSION ID]{HIJSON: cartographic document for web modeling 
...
The Linear Algebraic Representation (LAR) scheme uses Combinatorial Cellular Complexes (CCC) as mathematical domain [4] and various compressed representations of sparse matrices [9] as codomain.
...
In order to guarantee a standard quality of datum for any generated
...
points are removed, and a 1-dimensional LAR data structure is generated,
...
In particular, let us consider the binary characteristic matrix \texttt{EV}, and remark that we represent the edges on the
...
\]
Therefore, the columns of $\texttt{EV}$, i.e.\textasciitilde{}the
rows of $\texttt{VE} := \texttt{EV}^\top$, after being sorted
on their angles $\alpha$, and associated with the angle differences
$\Delta\alpha$, will provide a basis of elementary $1$-cochains that
evaluate to zero for each closed 1-cochain, i.e.~for every cycle
supported by the given line arrangment.
...
\begin{lstlisting}[language=json, label={lst:feature-collection-example}, captionpos=b,  caption=The traversal algorithm, producing a \texttt{HIJSON} file.]
def print_traversal(out,obj,parent_id='',level=0): 
  if is_struct(obj):
    ...
    print_traversal(out, obj.body, obj.name)
    return
  if is_model(obj):
...
\subsubsection{Cross-storey user navigation} The HIJSON Framework also provides
